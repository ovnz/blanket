...
local function gauge_graph(flip)
...
local function score_info(flip)
    local bg_x, bg_w = 0, 463
...
local function misscount_combo(flip)
    local bg_x, bg_w = 0, 463
...
local function judge_detail(flip)
...
local function fs_combo_break(flip, fast_w, slow_w)
    local bg_x, bg_w = 0, 463
...
local function main_pane(t)
    ...
    return {
        ...
        SkinObject:new(score_info(flip),      24, 405),
        SkinObject:new(misscount_combo(flip), 24, 319),
        SkinObject:new(judge_detail(flip),    24, 110),
        SkinObject:new(fs_combo_break(flip, t.fast_w, t.slow_w),  24,  24),
    }
end
